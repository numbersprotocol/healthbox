...
export class RewardService {
  daysRecorded$ = this.dataStore.recordsByDate$.pipe(
    map(recordsByDate => Object.keys(recordsByDate).length)
  );
  private readonly initRewardStatusRefresher = new BehaviorSubject(0);
  initRewardStatus$ = combineLatest([
    this.daysRecorded$,
    this.initRewardStatusRefresher,
  ]).pipe(
    switchMap(([days, _]) =>
      this.canGetInitialBalance().pipe(
        catchError(() => of(null)),
  ...
  canGetInitialBalance(): Observable<boolean> {
    return this.login().pipe(
      switchMap(userAuth =>
        this.privateCouponService.canGetInitialBalance(userAuth)
      )
    );
  }
  ...
  createUserCredential(email: string): Observable<UserCredential> {
    return this.dataStore.userData$.pipe(
      take(1),
      switchMap(userData =>
        this.privateCouponService.createUserCredential(email, userData.uuid)
      )
    );
  }
  ...
  getUserCredential(): Observable<UserCredential> {
    return this.dataStore.userData$.pipe(
      take(1),
      switchMap(userData =>
        this.privateCouponService.createUserCredential(
          userData.email,
          userData.uuid
        )
      )
  ...
  getInitReward(): Observable<any> {
    return this.login().pipe(
      switchMap(userAuth =>
        this.privateCouponService.giveInitialBalanceIfAvailable(userAuth)
      )
    );
  }

  login(): Observable<UserAuth> {
    return this.getUserCredential().pipe(
      switchMap(userCredential =>
        this.privateCouponService.login(userCredential)
      )
    );
  }
  ...
  signup(email: string): Observable<UserData> {
    return this.createUserCredential(email).pipe(
      switchMap(userCredential =>
        this.dataStore
          .updateUserData({
      ...
      switchMap(userCredential =>
        this.privateCouponService.signup(userCredential)
      ),
  ...
  private updatePoolBalance(): Observable<number> {
    return this.privateCouponService
      .getPoolCurrentBalance()
      .pipe(tap(value => this.poolBalance.next(value)));
  }

  private updateUserBalance(): Observable<number> {
    return this.login().pipe(
      switchMap(userAuth =>
        this.privateCouponService.getUserCurrentBalance(userAuth)
      ),
