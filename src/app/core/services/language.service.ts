...
export class LanguageService {
  ...
  language$ = this.dataStore.userData$.pipe(
    map(userData =>
      userData.language ? userData.language : this.browserLanguage
    )
  );
  ...
  set(language: string): Observable<any> {
    return this.translateService
      .use(language)
      .pipe(switchMap(() => this.setUserDataLanguageIfDifferent(language)));
  }
    ...
    return this.dataStore.userData$.pipe(
      ...
      switchMap(userData =>
        iif(
          ...
          this.dataStore.updateUserData({ language }),
          of([])
        )
      )
    );
