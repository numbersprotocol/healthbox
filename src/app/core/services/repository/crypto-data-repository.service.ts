...
export class CryptoDataRepositoryService {
  ...
  get(): Observable<CryptoData> {
    return this.localStorage.getData(
      this.CRYPTO_DATA_REPOSITORY,
      this.defaultCryptoData
    );
  }

  save(cryptoData: CryptoData): Observable<CryptoData> {
    return this.localStorage
      .setData(cryptoData, this.CRYPTO_DATA_REPOSITORY)
      .pipe(tap((u) => this.cryptoData.next(u)));
  }

  resetToDefault(): Observable<CryptoData> {
    return this.localStorage
      .setData(this.defaultCryptoData, this.CRYPTO_DATA_REPOSITORY)
      .pipe(tap((u) => this.cryptoData.next(u)));
  }
}
