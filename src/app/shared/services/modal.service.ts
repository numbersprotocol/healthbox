...
import { AddRecordComponent } from '@shared/components/add-record/add-record.component';
...
export class ModalService {
  ...
  showAddPhotoModal(): Observable<any> {
    ...
    ).pipe(switchMap(modal => this.presentModal(modal)));
  }
  ...
  showModal(component: any, componentProps?: any): Observable<any> {
    return this.createModal(component, componentProps).pipe(
  ...
  private presentModal(modal: HTMLIonModalElement): Observable<any> {
    return defer(() => modal.present()).pipe(
